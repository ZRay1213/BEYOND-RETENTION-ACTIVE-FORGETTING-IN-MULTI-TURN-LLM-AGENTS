...
# =============================================================================
ACTION_LIST = ['CONTINUE', 'VERIFY', 'CLARIFY', 'RESET', 'CONCLUDE', 'INJECT_SUMMARY']

DIRECTIVE_TEMPLATES = {
    'CONTINUE': None,
    'VERIFY': (
        'Important directive: Before proceeding, verify your previous numerical answer step-by-step '
        'using only the facts explicitly stated by the user. Recompute any arithmetic if necessary. '
        'Correct any inconsistencies found.'
    ),
    'CLARIFY': (
        'Important directive: Before providing an answer, ask the user ONE concise clarifying question '
        'about the most recent fact needed to solve the problem. Reply with only the question.'
    ),
    'RESET': (
        'Important directive: Discard any partial answers given previously. Re-read all user-provided '
        'information from the start, list each fact, and propose a fresh approach. Drop any claims '
        'that contradict new facts.'
    ),
    'CONCLUDE': (
        'Important directive: Cease seeking additional information. Commit to your best final numerical '
        'answer immediately in the form \\boxed{...} based solely on the information available.'
    ),
...
CONTROLLER_SYSTEM = (
    ...
    "Output EXACTLY one word: CONTINUE | VERIFY | CLARIFY | RESET | CONCLUDE | SUMMARY"
)
...
def controller_select_action(state, history_str):
    ...
    try:
        ...
        for a in ['CONTINUE', 'VERIFY', 'CLARIFY', 'RESET', 'CONCLUDE', 'SUMMARY']:
            if a in raw:
...
if __name__ == '__main__':
    ...
    samples = [s for s in samples if s['task'] == 'math']
    print(f'Loaded {len(samples)} samples; conv_type={args.conv_type}')

    todos = []
    rc = Counter(get_run_counts(args.conv_type, 'math', ASSISTANT_MODEL, args.dataset_file, log_folder=args.log_folder))
    for s in samples:
