...
class FreshEverySimulator(ConversationSimulatorSharded):
    def __init__(self, sample, log_folder, dataset_fn=None, conv_type='fresh_every'):
        super().__init__(
            sample,
            assistant_model=ASSISTANT_MODEL,
            system_model='qwen2.5-7b-tool',
            user_model='qwen2.5-7b-tool',
...
def run_fresh_every(sample, log_folder, dataset_fn=None, conv_type='fresh_every'):
    sim = FreshEverySimulator(sample, log_folder, dataset_fn=dataset_fn, conv_type=conv_type)
    sim.run()
...
if __name__ == '__main__':
    ...
    def _run(s):
        try:
            run_fresh_every(s, args.log_folder, dataset_fn=args.dataset_file, conv_type=args.conv_type)
