"""MARKED-HISTORY experiment.

Keep all LLM prior assistant messages in context, but prefix each with an explicit
epistemic marker indicating they are outdated partial-info commitments.

If the gap recovery is comparable to Fresh-Last, format/instruction-following is the
main mechanism (H3). If it shows only partial recovery, attention anchoring is the
dominant mechanism (H1) — explicit text marker can't override learned attention pattern.
"""
import os
...
EPISTEMIC_MARKER = "[OUTDATED PARTIAL-INFO COMMITMENT — only partial information was available when this was generated, do not rely on it]\n"


class MarkedHistorySimulator(ConversationSimulatorSharded):
    def __init__(self, sample, log_folder, dataset_fn=None, conv_type='marked_history'):
        super().__init__(
            sample,
    ...
    def _mark_assistant_messages(self, conversation):
        """Take a list of {role,content} messages, mark assistant messages
        except the very last assistant message (which has not been generated yet)."""
        marked = []
        asst_indices = [i for i, m in enumerate(conversation) if m.get('role') == 'assistant']
        # mark all but possibly the last (current call has no asst yet, but we mark all that exist)
        marked_set = set(asst_indices)
        for i, m in enumerate(conversation):
            if i in marked_set:
                marked.append({**m, 'content': EPISTEMIC_MARKER + m.get('content', '')})
            else:
                marked.append(m)
        return marked
    ...
    def run(self, verbose=False, save_log=True):
        ...
        while not is_completed:
            ...
            conversation = self._mark_assistant_messages(conversation)
            if is_last_turn:
                self.trace.append({'role': 'log', 'content': {'type': 'marked-history', 'turn': 'last', 'marker_count': sum(1 for m in conversation if m.get('role')=='assistant')}, 'timestamp': date_str()})
            resp_obj = generate(
                conversation,
...
def run_marked_history(sample, log_folder, dataset_fn=None, conv_type='marked_history'):
    sim = MarkedHistorySimulator(sample, log_folder, dataset_fn=dataset_fn, conv_type=conv_type)
    sim.run()
...
if __name__ == '__main__':
    ...
    p.add_argument('--log_folder', default='logs_marked_history')
    p.add_argument('--conv_type', default='marked_history')
    args = p.parse_args()
    ...
    def _run(s):
        try:
            run_marked_history(s, args.log_folder, dataset_fn=args.dataset_file, conv_type=args.conv_type)
