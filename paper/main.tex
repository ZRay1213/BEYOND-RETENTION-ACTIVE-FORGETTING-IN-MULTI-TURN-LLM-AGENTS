...
The dominant abstraction for LLM agent memory---across KV cache compression in serving stacks~\citep{kwon2023vllm,zheng2024sglang,liu2025lmcache,zhang2023h2o,xiao2024streamingllm,li2024snapkv}, hierarchical memories in agent frameworks~\citep{packer2023memgpt,li2025memos,park2023generative,memagent2026}, and summarization-based long-term memory~\citep{wu2023recursivesumm}---treats stored content as authoritative evidence to be retrieved or compressed. This abstraction inherits from databases and information retrieval, where every datum was externally observed. LLM-generated content does not satisfy this assumption: an assistant response is a model action, often a provisional commitment under a strict subset of the eventual evidence. Persisting it under the same chat-template role as user evidence biases subsequent decisions. We argue this is a \emph{category error}. The missing operation is \emph{active forgetting}: content-typed eviction at decision time, complementing generation and retention. Where retention answers ``can this prefix be reused?'', forgetting answers ``should this content remain eligible for future decisions?''. Existing serving systems are intentionally agnostic to the latter; we show this agnosticism is not a benign optimization but a correctness-bearing omission.
...
\begin{itemize}\itemsep0pt
    \item \textbf{A category-level claim.} We identify the missing operation in LLM agent memory as \emph{active forgetting} of provisional content, distinct from generation and retention. Existing serving and memory systems treat all stored content as authoritative evidence; LLM-generated content is structurally not, and persisting it under the same role biases later decisions. This is a category error, not an optimization gap.
    \item \textbf{A regime-independent theory.} Definition~\ref{def:provisional} formalizes provisional content; Proposition~\ref{prop:cliff} shows any non-empty subset of provisional content perpetuates posterior contamination. The dose-response cliff at $K{=}1.0$, the marked-history null, and the universal failure of attention-salience compression all follow as corollaries.
    \item \textbf{A taxonomy of forgetting operations} (Section~\ref{sec:taxonomy}, Table~\ref{tab:forget-taxonomy}): token-attention-level, sentence-level, turn-level uniform, block-typed, conversation-level. Each operator's domain of validity is specified by what content it can correctly classify.
    \item \textbf{Empirical validation across regimes and models.} On Lost-in-Conversation, \textsc{Fresh-Every} reaches 103\% gap recovery and exceeds the single-turn ceiling; the cliff and the marked-history null replicate across five models in four architecture families (Qwen-7B/14B, Llama-3.1-8B, Mistral-7B, Gemma-2-9B). On BFCL v4 stateful tool use, uniform forgetting collapses ($-20.5$\,pp vs.\ \textsc{Sharded}, $N{=}200$) while typed \textsc{CacheGuard} matches the strongest baseline---direct evidence that the operator must be content-typed.
    \item \textbf{Mechanism.} Prior assistant commitments receive per-token attention comparable to the system prompt and above user evidence; \texttt{[OUTDATED]} markers leave accuracy at the floor. The mechanism is attention-mediated rather than format-mediated, ruling out KV compression by attention salience as a fix.
    \item \textbf{Three metrics for forgetting} (PCHR, VCHR, HCHR) and a deployable instantiation (\textsc{CacheGuard}, no retraining, client-side). Both make the abstract claim measurable and shippable.
\end{itemize}
...
\small
\begin{tabular}{lp{0.32\linewidth}p{0.42\linewidth}}
\toprule
...
Sentence-level & Drop specific sentences within a turn & Intra-turn reasoning (Thought Anchors~\citep{thoughtanchors2025}); cross-turn extension in Appendix~\ref{app:anchor-id}. \\
Turn-level (uniform) & Drop all prior assistant turns at decision time (\textsc{Fresh-Last}/\textsc{Fresh-Every}) & Stateless reasoning where every assistant turn is provisional. Fails when some assistant content carries verified state. \\
Block-typed & Drop blocks classified as provisional, retain authoritative (\textsc{CacheGuard}) & Mixed regimes with both kinds of content (BFCL v4: tool calls + tool results are authoritative, asst-text is provisional). \\
Conversation-level & Reset all context per query (\textsc{Concat}) & Bounded single-shot evaluation; loses history information. \\
\bottomrule
\end{tabular}
\end{table}
...
Proposition~\ref{prop:cliff} is not specific to the multi-turn underspecified setting. Any agent regime that feeds model-generated content back as context---multi-turn chat, agentic step--observation loops, long chain-of-thought, summarization-based memory~\citep{wu2023recursivesumm}, iterative retrieval-augmented refinement---produces provisional content in the sense of Definition~\ref{def:provisional}. The corollary is that any compression or eviction policy that leaves a non-empty subset of provisional content in the posterior fails: heuristics such as ``keep recent $k$ turns'' or ``keep attention-important tokens'' (H2O, SnapKV, StreamingLLM) interpolate between contaminated states. Only an operator that types content by epistemic role---authoritative vs.\ provisional---and evicts the latter exits the contaminated regime. We empirically demonstrate this prediction on multi-turn underspecified chat (Section~\ref{sec:main-protocol}); cross-regime evidence in agentic loops, long CoT, and summarization-memory is a forward-looking direction (Section~\ref{sec:limitations}).
