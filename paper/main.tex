...
\paragraph{Cross-regime empirical: summarization-based memory inherits contamination.}
We directly test the summarization-memory prediction on $n{=}12$ failed sharded math conversations (Qwen2.5-14B). Three conditions: (i) \textsc{Sharded} baseline (full history), (ii) \textsc{Fresh-Last} (drop all prior assistant turns), (iii) \textsc{Summary-Memory} where we prompt the model to summarize the prior history into a single paragraph and then run generation on \texttt{[system + summary + final user]}. If summarization successfully strips contamination, \textsc{Summary-Memory} should approach \textsc{Fresh-Last}; if the summary inherits the contamination (because the summary itself is provisional content produced by the same model under partial information), \textsc{Summary-Memory} should approach \textsc{Sharded}.
\begin{center}
\small
\begin{tabular}{lc}
\toprule
...
\midrule
\textsc{Sharded} (full history) & 0.333 \\
\textsc{Fresh-Last} (truncate) & \textbf{0.417} \\
\textsc{Summary-Memory} (replace history with summary) & 0.333 \\
\bottomrule
\end{tabular}
\end{center}
\noindent\textsc{Summary-Memory} matches \textsc{Sharded} exactly and fails to reach \textsc{Fresh-Last}---the summary inherits the contamination. The summary text contains the same fabricated commitments and provisional answers as the original assistant turns; replacing many tokens with fewer tokens does not change the epistemic type of those tokens. The cliff prediction holds: a forgetting operator that compresses but does not type cannot escape contamination. This rules out summarization-based memory frameworks~\citep{wu2023recursivesumm,packer2023memgpt,lee2024readagent} as solutions to the multi-turn anchoring problem in the absence of typed eviction.

We demonstrate the prediction directly on multi-turn underspecified chat (Section~\ref{sec:main-protocol}), on stateful tool use (Section~\ref{sec:bfcl}), and on summarization-based memory above. Empirical validation in the remaining regimes (agentic loops, long CoT, persona drift) is the highest-priority follow-up (Section~\ref{sec:limitations}).
...
\paragraph{HCHR empirical (n=40).}
The pilot HCHR $= 0.16$ on $n{=}5$ reported in earlier versions of this work is refined here. On $n{=}40$ failed sharded conversations (Qwen2.5-14B on math and data2text; per-block ablation: each prior assistant block individually removed, score regraded), mean HCHR $= 0.069$ with 95\% bootstrap CI $[0.018, 0.130]$ (CI excludes zero, so HCHR is non-trivially nonzero but small). 6 of 40 conversations contain at least one harmful block; the per-position distribution is non-uniform: turn 0 is never harmful ($0/40$), peaks at turns 2--3 ($0.10$--$0.13$), and the small-$n$ deep-turn rate (turn 5: $0.29$, $n{=}7$) suggests later commitments more often flip the outcome. This is the expected pattern under Proposition~\ref{prop:cliff}: any single block individually rarely lifts a contaminated conversation (because the other retained provisional content also contributes a likelihood factor), but as turns accumulate any single block becomes proportionally more likely to be the dominant anchor. The cliff explanation predicts low HCHR (most single blocks do not solo-flip) but non-zero HCHR (in some cases a single block is the dominant anchor), which is what we observe.
...
\section{Synthetic benchmark results (F1, F2, F3)}
\label{app:synthetic}

F1-additive poses a multi-turn underspecified math task: across 4--6 turns, the user reveals constraints on a positive integer (divisibility, parity, bounds, digit-sum); the assistant must commit to the smallest integer satisfying all revealed constraints. Every assistant turn before the final shard is, by construction, a partial-info commitment. F2-corrective requires the assistant to track explicit user retractions (``Actually, the limit is X, not Y''), creating an even simpler anchoring scenario where the final correct constraint directly contradicts the partial one. F3-artifact constructs a multi-step word problem in which the assistant produces an intermediate \emph{artifact} (a numerical sub-result) that is then required at a later turn---the assistant must propagate, not regenerate. Each of the three benchmarks reproduces a substantial multi-turn drop and confirms that removing \emph{all} provisional commitments fully recovers accuracy.

\begin{table}[h]
\centering
\caption{F1, F2, F3 synthetic benchmarks (Qwen2.5-14B, $N=2$ sims/instance, $n{=}30$ instances each). F1: additive constraints. F2: corrective retraction. F3: intermediate-artifact propagation. Each benchmark reproduces a multi-turn drop fully closed by any stateless protocol. F3 produces the largest absolute drop (Concat 1.000, Sharded 0.433) because the artifact-propagation structure is the most sensitive to provisional content carrying into later turns.}
\label{tab:f1f2f3}
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{2}{c}{F1 (additive)} & \multicolumn{2}{c}{F2 (corrective)} & \multicolumn{2}{c}{F3 (artifact)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
Protocol & Acc & Gap rec. & Acc & Gap rec. & Acc & Gap rec. \\
\midrule
Concat (ceiling) & 0.977 & 100\% & 1.000 & 100\% & 1.000 & 100\% \\
Sharded (floor) & 0.609 & 0\% & 0.767 & 0\% & 0.433 & 0\% \\
\midrule
\textbf{Fresh-Last} & 1.000 & 106\% & 1.000 & 100\% & 1.000 & 100\% \\
\textbf{Fresh-Every} & 1.000 & 106\% & 1.000 & 100\% & 1.000 & 100\% \\
\textbf{CacheGuard} & 1.000 & 106\% & 1.000 & 100\% & 1.000 & 100\% \\
\bottomrule
\end{tabular}
\end{table}

On all three F-benchmarks every stateless protocol attains 1.000. F3 in particular drives the Sharded floor to 0.433 (a 57\,pp drop)---larger than F1 and F2---because the artifact-propagation structure means earlier provisional outputs are not just shaping later answers but are \emph{required as direct inputs} to them. The cliff theorem (Proposition~\ref{prop:cliff}) predicts this: when a provisional output is treated as factual input downstream, retention of the wrong artifact is maximally harmful. F3 does not differentiate \textsc{CacheGuard} from \textsc{Fresh-Every}, because the artifact in this synthetic test is the model's own output (provisional), not external state. The BFCL regime (Section~\ref{sec:bfcl}) is the stress-test where tool results represent irreducible verified state that must be retained, and where typed eligibility becomes structurally necessary.
...
\begin{itemize}\itemsep0pt
    \item \textsc{baseline}: standard generation, attention mask of all ones (the rendered sharded context).
    \item \textsc{patch-asst}: attention mask zeroed at all token positions belonging to prior assistant turns. Generation queries cannot attend to those positions.
    \item \textsc{patch-user}: same construction, but applied to user-evidence positions instead (control). Removing valid evidence should not recover accuracy.
\end{itemize}
