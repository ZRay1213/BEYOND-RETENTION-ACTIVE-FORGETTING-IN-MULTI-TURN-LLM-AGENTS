...
\title{Beyond Retention: \\ Active Forgetting in Multi-Turn LLM Agents}
...
% =====================================================
\begin{abstract}
Multi-turn LLM degradation on underspecified conversations is largely an artifact of the chat protocol, not a model-capability deficit. On Lost-in-Conversation with Qwen2.5-14B, a five-line policy that drops the model's own prior tokens at the final turn (\textsc{Fresh-Last}) recovers 87\% of the multi-turn gap; dropping them at every turn (\textsc{Fresh-Every}) reaches 103\% and exceeds the single-turn ceiling. The fix is binary rather than graded: keeping any 25--75\% of prior assistant tokens recovers only 4--23\% of the gap. The mechanism is attention-mediated. On $n{=}22$ failed sharded conversations (Qwen2.5-7B), prior provisional commitments receive per-token attention comparable to the system prompt and 8.5\% above user evidence, so attention-salience-based KV compression preferentially keeps tokens that should be evicted. Text-level mitigations also fail: prefixing prior tokens with \texttt{[OUTDATED]} barely moves accuracy. The effect replicates across five models in four architecture families (Qwen-7B/14B, Llama-3.1-8B, Mistral-7B, Gemma-2-9B). We frame this as a missing memory primitive. Agent systems implement \emph{generation} and \emph{retention} (KV cache reuse) but lack \emph{active forgetting}. We introduce VCHR and HCHR alongside the standard prefix-cache hit rate (PCHR; message-level proxy mPCHR) to make forgetting measurable, and propose \textsc{CacheGuard}, a typed forgetting controller that drops provisional commitments while retaining verified evidence. \textsc{CacheGuard} validates on all four architecture families (Qwen-14B 0.875, Llama-3.1-8B 0.704, Mistral-7B 0.544, Gemma-2-9B 0.741), and matches or exceeds \textsc{Fresh-Every} on Mistral and Gemma. The picture reverses cleanly on stateful tool use (BFCL v4): \textsc{Fresh-Every} collapses to 0.090 while \textsc{CacheGuard} matches the strongest baseline (0.280 vs.\ 0.295, $p{=}0.66$, ns), so forgetting must be content-typed rather than uniform. Chat protocols and serving stacks should expose active forgetting as a first-class primitive, alongside generation and retention.
\end{abstract}
...
% --- Para 1: Active forgetting is the missing memory operation ---
LLM agent systems are designed around two memory operations: \emph{generation} (the agent's outputs) and \emph{retention} (KV-cache reuse, optimized in modern serving stacks like vLLM~\citep{kwon2023vllm}, SGLang~\citep{zheng2024sglang}, and LMCache~\citep{liu2025lmcache}, monitored via prefix-cache hit statistics). A third operation has received much less attention: \emph{active forgetting}. Standard chat protocols have no forgetting mechanism beyond full-history truncation: a user message (external evidence) and a prior model output (often a provisional commitment under partial information) both persist indefinitely, serialized under identical role tags. Existing serving systems are intentionally agnostic to the epistemic status of cached content---they answer ``can this prefix be reused?'' but not ``should this content remain eligible for future decisions?''. The latter question is what forgetting answers.
...
\begin{itemize}\itemsep0pt
    \item \textbf{VCHR} (valid cache hit rate): of reused tokens, the fraction belonging to blocks that remain epistemically valid evidence under the current state.
    \item \textbf{HCHR} (harmful cache hit rate): of reused tokens, the fraction whose presence flips the answer from correct to wrong (counterfactually).
\end{itemize}
...
\begin{itemize}\itemsep0pt
    \item \textbf{Cliff, not gradient.} Dropping 25--75\% of prior assistant messages recovers only 4--23\% of the gap. Only complete removal ($K=1.0$, \textsc{Fresh-Last}) recovers 87\%; no partial compromise works.
    \item \textbf{Capability inversion.} Qwen2.5-7B under \textsc{Fresh-Last} reaches 0.917, nominally above 14B \textsc{Fresh-Last} (0.771) and the 14B Concat ceiling (0.854). Larger models generate more coherent commitments under partial information, producing stronger anchors rather than resistance. The nominal exceedance is partially attributable to multi-attempt advantage on Qwen-7B (Appendix~\ref{app:bestofN}); on Llama-3.1-8B the same ordering survives a best-of-$N$ control.
    \item \textbf{Attention, not format.} Marking prior messages with \texttt{[OUTDATED PARTIAL-INFO COMMITMENT]} yields 0.307, barely above the 0.208 baseline and far below \textsc{Fresh-Last} (0.771). Text markers do not override attention-mediated anchoring.
    \item \textbf{Task-structure dependent.} \textsc{Recap} recovers 86\% of the gap on tool-use actions but only 27\% on code generation (Table~\ref{tab:main}); single-call commitments are easier to partially override than multi-step ones.
\end{itemize}
...
\begin{itemize}\itemsep0pt
    \item \textbf{Framing.} We position \emph{active forgetting} as a missing primitive in LLM agent memory, alongside generation and retention. Three metrics (PCHR, VCHR, HCHR) operationalize it: PCHR measures retention, VCHR measures whether reused content is still valid evidence, HCHR measures whether reused content is causally harmful.
    \item \textbf{Diagnosis.} \textsc{Recap} and \textsc{Fresh-Last} differ only in whether prior assistant tokens are forgotten at decision time, yet \textsc{Fresh-Last} recovers 22\,pp more of the gap. The dose-response is a cliff at $K{=}1.0$, a ceiling exceedance survives a best-of-$N$ control on Llama-3.1-8B (Appendix~\ref{app:bestofN}), and \texttt{[OUTDATED]} markers leave accuracy at 0.307 vs.\ 0.771 for full removal. Forgetting is binary, attention-mediated, and not overridable by text instructions.
    \item \textbf{Method.} \textsc{CacheGuard} classifies context blocks by epistemic role, drops provisional commitments, and retains verified evidence. It needs no retraining and is deployable client-side. The forgetting effect and \textsc{CacheGuard} both replicate across five models in four architecture families (Qwen-7B/14B, Llama-3.1-8B, Mistral-7B, Gemma-2-9B); on Mistral and Gemma \textsc{CacheGuard} matches or exceeds \textsc{Fresh-Every}, showing that typed eligibility can outperform uniform forgetting.
    \item \textbf{Two-regime validation.} On LiC (stateless reasoning) \textsc{Fresh-Every} reaches 0.875. On BFCL v4 (stateful tool use) \textsc{Fresh-Every} collapses to 0.090 ($-20.5$\,pp vs.\ \textsc{Sharded}, $p<0.001$, $N=200$). \textsc{CacheGuard} matches \textsc{Sharded} on BFCL (ns) while still beating it on LiC, so forgetting must be content-typed rather than uniform.
\end{itemize}
...
\citet{huang2026benefit} report that omitting prior assistant responses preserves quality and reduces context up to 10$\times$ on in-the-wild conversations, identifying ``context pollution'' on 36.4\% of self-contained turns. We extend this in four ways: (i) controlled sharded benchmark where the failure is reproducible by construction; (ii) a binary-cliff mechanism showing partial filtering does not work; (iii) formalization as a \emph{cache validity} problem with three metrics connecting to the serving stack; (iv) \textsc{CacheGuard}, a typed reuse layer that handles the stateful-tool reversal (Section~\ref{sec:bfcl}) where assistant-history deletion is harmful. Three concurrent papers identify the same anchoring phenomenon under different names. \citet{wang2025proactive} introduce PI-LLM, showing log-linear decay in retrieval accuracy under proactive interference even when the target value is adjacent to the query; their key result---``ignore earlier inputs'' instructions fail---directly corroborates our marked-history null. \citet{li2026inertia} call the effect ``conversational inertia'' and propose periodic blind context clipping; \citet{zhang2026rlanchor} address it via expensive RL fine-tuning. \citet{xu2026sleepgate} build a learned sleep-cycle gate inside the KV cache. Compared with these, our contribution is the cliff mechanism, the typed-vs-untyped reversal on stateful tool use, and a zero-shot, training-free, deployable controller.

\paragraph{Active forgetting in cognitive architectures and agent memory.}
Forgetting is a first-class operation in cognitive architectures: directed forgetting~\citep{bjork1972directedforgetting} and ACT-R activation decay~\citep{anderson1998actrarch} explicitly evict working-memory contents. Anchoring bias in human judgment~\citep{tversky1974anchoring}---where numerical estimates remain biased toward an initial value even when that anchor is identified as arbitrary---provides a behavioral analog to commitment contamination: our LLM-level effect differs in that the anchor is not arbitrary but is the model's own prior response, making it a self-reinforcing rather than externally imposed bias. Recent LLM agent systems revive this lineage~\citep{park2023generative,packer2023memgpt,li2025memos,kosinski2025actr,memagent2026} with hierarchical memories and importance-based eviction, but eviction is treated as a pragmatic optimization rather than a correctness-bearing primitive. Our framing sharpens this for chat-protocol multi-turn settings: forgetting is content-typed (provisional commitment vs.\ verified evidence) and is structurally necessary, not optional. We distinguish active forgetting at \emph{inference time} on the KV cache from machine unlearning, which operates on \emph{training-time weights}; these address categorically different failure modes (episodic-context bias vs.\ parametric-knowledge erasure). \citet{khattab2025contexteng} survey context engineering; our work fills the missing typed-eviction module in their taxonomy.
...
We type each non-log message in the conversation:
\begin{itemize}\itemsep0pt
    \item \texttt{system} --- protocol instruction (always valid).
    \item \texttt{user\_evidence} --- user-revealed shard (always valid).
    \item \texttt{tool\_observation} --- successful tool result (always valid).
    \item \texttt{assistant\_artifact} --- a code block, table, or named output produced by the assistant.
    \item \texttt{assistant\_claim} --- an answer attempt or boxed/bracketed final answer.
    \item \texttt{assistant\_clarification} --- a question to the user.
    \item \texttt{assistant\_plan} --- a description of intended next steps without producing artifact.
\end{itemize}
We additionally track \texttt{contradicted\_by} (blocks invalidated by a later block) and \texttt{referenced\_by} (artifacts referenced by later user or tool blocks).

\paragraph{Eligibility Controller.}
A block $b$ is reuse-eligible iff:
\begin{itemize}\itemsep0pt
    \item it is system, user, or successful tool; or
    \item it is an artifact and at least one later user/tool block references it; or
    \item it is a clarification (asking, not committing).
\end{itemize}
...
\paragraph{Why is the dose-response a cliff? A formal account.}

\begin{proposition}[Commitment-Contamination Cliff]
\label{prop:cliff}
Let $y$ be the correct answer, $\mathcal{E}$ the full user-revealed evidence at decision time, and $a_{1:T}$ the sequence of prior assistant responses, each generated under a strict evidence subset $\mathcal{E}_{<t} \subsetneq \mathcal{E}$.
Under standard chat rendering, the LLM posterior decomposes as
\[
p(y \mid \mathcal{E},\,a_{1:T}) \;\propto\;
\underbrace{p(y \mid \mathcal{E})}_{\text{evidence-conditioned}} \cdot
\underbrace{\prod_{t=1}^{T} p(a_t \mid y,\,\mathcal{E}_{<t})}_{\text{commitment likelihood}}.
\]
For any retained subset $S \subseteq \{1,\ldots,T\}$ with $|S|>0$, the term $\prod_{t \in S} p(a_t \mid y,\mathcal{E}_{<t})$ persists and biases $p(y\mid\mathcal{E},a_S)$ whenever any $a_t$ committed to $y' \neq y$.
The unique uncontaminated fixed point is $S=\emptyset$---full assistant-history removal.
\end{proposition}

\begin{remark}[Textual reweighting cannot truncate]
\label{rem:textual}
Appending an ``this response is outdated'' annotation to $a_t$ changes the token distribution of the factor $p(a_t \mid y,\mathcal{E}_{<t})$ but leaves it non-zero.
The Marked-History null result (Section~\ref{sec:cacheguard}) is the empirical confirmation of this prediction.
\end{remark}

The cliff at $K=1.0$ is therefore not an empirical surprise: Proposition~\ref{prop:cliff} predicts it as the unique bias-free operating point.
Any retained assistant turn---regardless of position or recency---keeps the commitment likelihood active.
This is why common heuristics such as ``keep only the most recent $k$ turns'' or ``keep attention-important tokens'' fail: they leave at least one commitment factor in the posterior.
Partial truncation interpolates between contaminated states; only $S=\emptyset$ exits.

\begin{table}[t]
\centering
\caption{Selective-drop dose-response on multi\_heldout. $K$ is the fraction of prior assistant messages dropped. Drop-EARLIEST removes earliest $K$, Drop-LATEST removes latest $K$. $K=0$ is the standard Sharded protocol; $K=1.0$ Drop-EARLIEST is Fresh-Last. The cliff at $K=1.0$ dominates and replicates on Llama-3.1-8B (bottom block). Llama block uses $K\in\{0,0.5,1.0\}$ (three-point sweep); Qwen block uses four intermediate values.}
\label{tab:dose}
\begin{tabular}{lccc}
\toprule
...
\begin{itemize}[nosep]
\item \textbf{Stateless-reasoning regime (LiC):} prior assistant outputs are provisional partial-info claims; retaining them anchors later responses. Optimal: \textsc{Fresh-Every} (0.875). \textsc{CacheGuard} $\approx$ \textsc{Fresh-Last} (both drop unverified claims).
\item \textbf{Stateful-tool regime (BFCL):} prior tool results are verified external state; discarding them is catastrophic. Optimal: \textsc{Sharded} or \textsc{CacheGuard} (0.295/0.280, ns difference). \textsc{Fresh-Every} drops to 0.090.
\end{itemize}
...
\paragraph{Open-weight only; two benchmarks.}
Our cross-model evaluation covers five open-weight models (Qwen2.5-7B, Qwen2.5-14B, Llama-3.1-8B, Mistral-7B-Instruct-v0.2, Gemma-2-9B-it); generalization to closed-source families (GPT, Claude) is plausible but unverified. Primary results use LiC and BFCL v4, supplemented by F1/F2 synthetics. Cross-benchmark validation on $\tau^2$-Bench~\citep{barres2025tau2bench}, MultiChallenge~\citep{weston2025multichallenge}, and additional BFCL subsets is left to future work. The miss-param subset ($N=30$) shows no CacheGuard advantage (0.167 vs.\ Sharded 0.233), consistent with our prediction that CacheGuard targets tool-state retention, not function-spec robustness.
...
\paragraph{Approximate PCHR, unpopulated HCHR, and correlational attention.}
Our mPCHR is a message-level approximation to vLLM's token-level prefix cache hit rate; the two diverge for \textsc{Fresh-Every} where user content extends rather than rebuilds per turn. HCHR, as defined in Section~\ref{sec:metrics}, requires counterfactual per-block ablations to evaluate whether removing a block flips the outcome; this computation is expensive at scale. A five-conversation pilot on failed math sharded conversations (Qwen2.5-14B, $n=5$) finds mean HCHR $= 0.16$: individual block drops rarely recover accuracy, consistent with Proposition~\ref{prop:cliff}'s prediction that any retained commitment perpetuates contamination (partial removal fails; full removal succeeds). Full-scale empirical HCHR measurement is left for follow-up. The attention measurements in Section~\ref{sec:attention-probe} ($n=22$ failed sharded BFCL conversations) are correlational; causal interventions (attention patching, KV-block ablation) would more rigorously establish the anchoring mechanism but are left for follow-up.
...
\section{Synthetic benchmark results (F1 and F2)}
\label{app:synthetic}

F1-additive poses a multi-turn underspecified math task: across 4--6 turns, the user reveals constraints on a positive integer (divisibility, parity, bounds, digit-sum); the assistant must commit to the smallest integer satisfying all revealed constraints. Every assistant turn before the final shard is, by construction, a partial-info commitment. Concat ceiling is 0.977 and the Sharded floor is 0.609---a 37\,pp multi-turn drop that mirrors the LiC pattern. F2-corrective requires the assistant to track explicit user retractions (``Actually, the limit is X, not Y''), creating an even simpler anchoring scenario where the final correct constraint directly contradicts the partial one. Both benchmarks confirm that removing \emph{all} provisional commitments fully recovers accuracy.

\begin{table}[h]
\centering
\caption{F1 additive-constraints and F2 corrective-constraints synthetic benchmarks (Qwen2.5-14B, $N=2$ sims/instance). F1: user reveals additive constraints across 4--6 turns. F2: user gives an initial bound then explicitly retracts and corrects it. Both reproduce a multi-turn drop fully closed by any stateless protocol.}
\label{tab:f1f2}
\begin{tabular}{lcccc}
\toprule
& \multicolumn{2}{c}{F1 (additive)} & \multicolumn{2}{c}{F2 (corrective)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Protocol & Acc & Gap recovery & Acc & Gap recovery \\
\midrule
Concat (ceiling) & 0.977 & 100\% & 1.000 & 100\% \\
Sharded (floor) & 0.609 & 0\% & 0.767 & 0\% \\
\midrule
\textbf{Fresh-Last} & 1.000 & 106\% & 1.000 & 100\% \\
\textbf{Fresh-Every} & 1.000 & 106\% & 1.000 & 100\% \\
\textbf{CacheGuard} & 1.000 & 106\% & 1.000 & 100\% \\
\bottomrule
\end{tabular}
\end{table}

On both F1 and F2 every stateless protocol attains 1.000. This does not differentiate \textsc{CacheGuard} from \textsc{Fresh-Every}, because neither benchmark generates an assistant artifact that the user later references back. The BFCL regime (Section~\ref{sec:bfcl}) provides the stress-test where tool results represent irreducible verified state that must be retained.
...
\toprule
\textbf{Turn 1 (Asst, original \textsc{Sharded}):} ``\underline{To accurately calculate Ariadne's total sales for March and April, we need specific data regarding her sales for those months.} \underline{Since the actual sales figures are not provided in your question, I will outline the steps to calculate her total sales once you have the necessary data.} [\ldots steps follow\ldots]'' \\
\midrule
...
\midrule
\textbf{Turn 7 (Asst, after dropping either underlined sentence):} model now produces a numeric answer. Score: $1$. \\
\midrule
\textit{Anchor sentences} (both flip the outcome individually):
\begin{itemize}\itemsep1pt
  \item ``To accurately calculate Ariadne's total sales for March and April, we need specific data regarding her sales for those months.'' (Turn 1, sentence 0)
  \item ``Since the actual sales figures are not provided in your question, I will outline the steps to calculate her total sales once you have the necessary data.'' (Turn 1, sentence 1)
\end{itemize} \\
\bottomrule
\end{tabular}
\end{center}
